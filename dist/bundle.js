...
const vusaAssist = speech => {
  ...
  if (speech.match(openCmd)) {
    ...
    if (vusaOpens.hasOwnProperty(cmd[1])) {
      ...
      let lazyResponses = ["Can't you type, o lazy one?", "Wow you are lazy!", "One minute with me, and you've already forsaken the keyboard?", "What will you do without me, hmm?", "Why yes of course!"];
  ...
  if (speech.match(giphyCmd)) {
    ...
    __WEBPACK_IMPORTED_MODULE_0_axios___default.a.get(`https://api.giphy.com/v1/gifs/random?api_key=Ad45OKeJSw5AxL9F4QvcuAQnuZmIkK6w&tag=${giphy}&rating`).then(function (res) {
      return res.data;
    }).then(function (res) {
  ...
  if (!answered) {
    let unanswered = ["I'm sorry, what was that?", "I seem to be a bit slow today, can you repeat that?", "I didn't quite catch that...", "You talkin' to me?", "Err... what now?"];
